...
此功能特别适用于那些 IP 地址可能会变化的源站以及不能使用80 443端口访问的的网络，确保 CDN 始终能够正确地获取最新的内容。比如仅有动态 IPV6 地址的服务器，
...
#### windows
1. 安装 `pip install eodo2`
2. 运行 `eodo2 -p 54321`以启动。 -p 54321 表示 web 管理界面监听端口为 54321。
3. 在 `http://localhost:54321` 中配置必要的配置项。

#### ubuntu
1. 安装 `pip3 install eodo2 -U --break-system-packages`
2. 运行 `export PATH="$HOME/.local/bin:$PATH" && source ~/.bashrc` 将 ~/.local/bin 加入 PATH
3. 运行 `eodo2 -p 54321`以启动。  -p 54321 表示 web 管理界面监听端口为 54321。
